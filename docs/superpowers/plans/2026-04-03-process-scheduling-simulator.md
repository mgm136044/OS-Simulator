...
썸네일, 필터 적용, 고해상도 이미지, 4K 영상처럼 실행 시간이 크게 다른 작업이 섞여 들어오며, 프레임·타일 단위로 선점이 가능하고 해상도·프레임 수 기반으로 예상 BT를 추정할 수 있는 환경.

### 목적

> **실행 시간이 절반 이하 남은 프로세스를 레디큐 제일 앞에 배치하여 거의 완료된 작업의 불필요한 대기를 줄이고 평균 완료 시간을 단축함. hard real-time처럼 절대 마감 보장이 필요하지 않고 일부 지연을 허용하는 soft real-time 렌더링 서버에 적합.**
