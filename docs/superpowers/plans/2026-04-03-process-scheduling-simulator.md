...
- **다양한 BT**: 매칭 난이도에 따라 처리 시간이 다름 (솔로큐 vs 5인 파티)
- **공정성 필요**: 모든 유저의 매칭 요청이 일정 시간 내 처리되어야 함 (기아 방지)
- **완료 우선**: 이미 절반 이상 매칭 조건을 충족한 요청은 빨리 완료해야 유저 이탈 방지

### 목적

> **RR의 공정성과 SPN의 효율성을 절충하여, 진행률이 높은 작업의 완료 지연을 최소화한다.**
