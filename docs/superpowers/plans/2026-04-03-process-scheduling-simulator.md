...
### 대상 시스템: 이미지/영상 렌더링 서버

썸네일, 필터 적용, 고해상도 이미지, 4K 영상처럼 실행 시간이 크게 다른 작업이 섞여 들어오는 환경.

**실제 사례 — 유튜브 영상 업로드 처리 서버:** 영상이 업로드되면 썸네일(BT 짧음), 360p/720p 저화질 인코딩(BT 중간), 4K HDR 인코딩(BT 매우 김) 작업이 동시에 큐에 쌓인다. 사용자가 영상을 바로 볼 수 있으려면 썸네일과 저화질 미리보기가 먼저 완료되어야 한다. 거의 완료된 작업을 앞으로 당기는 Thanos 철학이 이 워크로드와 정확히 맞아 떨어진다.

**이 시스템이 Thanos에 적합한 이유:**

1. **남은 작업량을 미리 알 수 있다** — 1920×1080 이미지를 렌더링한다면 전체 픽셀 수가 처음부터 정해져 있고, 지금까지 처리한 픽셀 수를 카운트하면 남은 양을 바로 계산할 수 있다. "절반 남았다"는 조건을 실제로 측정할 수 있는 시스템이다.

2. **작업을 중간에 끊었다 다시 시작할 수 있다** — 렌더링은 픽셀·타일 단위로 나뉘어 처리되므로, 중간에 멈췄다가 다시 이어도 지금까지 계산한 결과를 버리지 않아도 된다. RR처럼 선점해도 손해가 없다.

3. **썸네일 하나와 4K 영상의 처리 시간 차이가 극단적이다** — FCFS로 짜면 4K 영상 하나가 큐를 막고, RR로 짜면 거의 끝난 작업도 계속 뒤로 밀린다. SPN은 처리 시간 예측이 틀리면 순서가 뒤집힌다. Thanos는 절반 이상 처리된 작업만 딱 한 번 앞으로 당겨서 세 문제를 동시에 피한다.

4. **새 요청이 쏟아져도 오래된 작업이 묻히지 않는다** — 신규 렌더 요청이 계속 들어와도, 절반 이상 처리된 작업은 반드시 한 번 우선권을 얻는다. 한 번 부스트받은 작업은 다시 부스트받지 않으므로(`boosted` set), 특정 작업이 계속 새치기하는 일도 없다.

5. **마감은 있지만 1초 늦는다고 시스템이 죽지는 않는다** — 영상 납품 마감처럼 시간 제약이 있지만, 항공기 제어처럼 1ms 초과 시 즉시 실패하는 환경은 아니다. 약간의 지연은 허용되므로, 평균 완료 시간을 줄이는 방향으로 스케줄링하면 충분하다.

6. **SPN보다 현실적으로 적용하기 유리하다** — SPN은 작업 시작 전에 실행 시간을 정확히 알아야 한다. 하지만 렌더링은 장면 복잡도·광원 수·해상도에 따라 실행 시간이 달라져 사전 예측이 어렵고, 예측이 틀리면 SPN의 순서 결정이 무너진다. Thanos는 실행 전 예측 없이 실행 중 직접 관측한 진행률을 기준으로 부스트를 결정하므로, 예측 오차에 흔들리지 않는 더 현실적인 방식이다.

7. **거의 완료된 작업을 먼저 끝내면 메모리도 빨리 비워진다** — RR은 모든 작업이 동시에 미완료 상태라 각 작업의 중간 렌더 버퍼가 메모리에 오래 남는다. Thanos는 거의 완료된 작업을 먼저 끝내 버퍼를 빨리 해제하므로, 동시에 메모리를 점유하는 작업 수가 줄어드는 경향이 있다. (단, 작업 종료 시 메모리가 즉시 반환되는 구조를 전제로 함)

**발표용 요약 (2~3줄):**

> 렌더링 서버는 작업 시작 전부터 전체 처리량을 알 수 있어 "절반 남았다"는 조건을 실제로 측정할 수 있다. 썸네일부터 4K 영상까지 작업 크기 차이가 극단적이고, 중간에 끊었다 이어도 결과를 버리지 않아 선점 스케줄링이 자연스럽게 맞아 떨어진다. Thanos는 이 환경에서 거의 완료된 작업을 딱 한 번 앞으로 당겨 평균 완료 시간을 줄이고 기아도 방지한다.

### 목적

> **실행 시간이 절반 이하 남은 프로세스를 레디큐 제일 앞에 배치하여 거의 완료된 작업의 불필요한 대기를 줄이고 평균 완료 시간을 단축. soft real-time 이미지/영상 렌더링 서버에서 활용 가능.**

| 비교 | RR | SPN | Thanos |
|------|:--:|:---:|:------:|
| 공정성 | 높음 | 낮음 (장기 작업 기아) | 높음 (RR 기반) |
| 평균 TT | 높음 | 낮음 | 중간 (부스트로 개선) |
| 응답성 | 좋음 | 나쁨 | 좋음 |
| 완료 효율 | 낮음 | 높음 | 중간~높음 |

### 핵심 아이디어 — "절반의 균형 (Half Balance)"
