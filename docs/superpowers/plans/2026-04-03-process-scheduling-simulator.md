...
1. **진행률 정량화 가능** — 렌더링은 타일·샘플·프레임 단위로 쪼개어 독립 계산하므로, 완료된 단위 수 / 전체 단위 수로 잔여 연산량(remaining_time)을 명확히 추정할 수 있다. `remaining_time ≤ burst_time/2` 조건이 현실적으로 측정 가능.

2. **프레임·타일 단위 선점 가능** — GPU 커널 단위가 아닌 렌더 청크 단위로 스케줄링하므로, RR 기반 선점이 현실적으로 적용 가능. 중단 후 재개 시 중간 결과(샘플 누적 등)를 재활용해 효율 손실이 낮다.

3. **작업 크기 편차가 큼** — FCFS는 큰 작업이 독점, RR은 거의 완료된 작업도 계속 밀림, SPN은 BT 추정 오차에 취약. Thanos는 절반 이상 진행된 작업을 1회 부스트해 이 세 문제를 동시에 완화.

4. **기아(Starvation) 방지** — 신규 작업이 계속 유입되어도 기존 작업이 절반 이상 진행되면 반드시 1회 우선권을 얻으므로, 거의 완료된 작업이 큐 뒤로 무한히 밀리는 현상을 방지. 1회 한정(`boosted` set)으로 부스트 남용도 차단.

5. **Soft real-time 적합성** — 납품 마감은 있지만 hard real-time처럼 즉각 실패하지 않으며 일부 지연을 허용. 평균 완료 시간 단축이 핵심 지표인 환경에 Thanos의 휴리스틱이 적합.
